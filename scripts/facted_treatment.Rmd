...
```{r}
library(readxl)
otw_data <- read_excel("exploratory/data/terminated_ongoing_trials_tabledata.xlsx")
View(otw_data)
```
...
```{r}
# Combine data from "otw_data" for In-progress and Terminated/Withdrawn trials
otw_data <- otw_data %>%
  mutate(
    AdjuvantSimplified = case_when(
      str_detect(`Adjuvant Therapy`, regex("Immunotherapy", ignore_case = TRUE)) ~ "Immunotherapy",
...
# Summarize In-progress and Terminated/Withdrawn
in_progress_data <- otw_data %>%
  filter(Status == "In-progress") %>%
  count(TimeGroup, AdjuvantSimplified) %>%
...
# Reformat treatmentinitiation_summary to match column names
completed_data <- treatmentinitiation_summary %>%
  rename(
    TimeGroup = YearGroup,
    AdjuvantSimplified = Treatment
  ) %>%
...
# Factor order for consistent stacking
...
ggplot(combined_trials, aes(x = TimeGroup, y = n, fill = AdjuvantSimplified)) +
  geom_bar(stat = "identity") +
...
final_plot <- ggplot(combined_trials, aes(x = TimeGroup, y = n, fill = AdjuvantSimplified)) +
  ...
  facet_wrap(~ StatusGroup, nrow = 1) +
  labs(
    title = "Number of Clinical Trials by Year and Concurrent Treatment",
    x = "Year",
    y = "Number of Trials",
...
# Save the plot
ggsave("combined_trials_by_year_and_treatment.png", plot = final_plot, width = 12, height = 6, dpi = 300, bg = "white")
